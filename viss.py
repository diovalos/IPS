...
SERIAL_PORT = 'COM4'
BAUD_RATE = 115200
LOG_FILE = "esp32_trilateration_log.txt"  # <== log file path
...
def connect_serial_once():
    ...
    try:
        ...
        print("⚠️ Failed to connect to serial port.")
        return None
...
def log_to_file(distances, position):
    """Append distances and position with timestamp to log file"""
...
def init():
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title("ESP32 Wi-Fi Trilateration Tracker")
    ax.grid(True)
    ax.legend()
...
if __name__ == "__main__":
    ...
    with open(LOG_FILE, "w") as f:
        f.write("=== ESP32 Wi-Fi Trilateration Log ===\n")
        f.write("Timestamp | Distances [narzo, hello, A] | Position (X,Y)\n\n")
